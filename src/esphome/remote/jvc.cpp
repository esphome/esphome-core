...
JVCTransmitter::JVCTransmitter(const std::string &name, uint32_t data)
    : RemoteTransmitter(name), data_(data) {}

void JVCTransmitter::to_data(RemoteTransmitData *data) {
  encode_jvc(data, this->data_);
}
...
JVCReceiver::JVCReceiver(const std::string &name, uint32_t data)
