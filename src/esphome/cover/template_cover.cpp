...
void TemplateCover::loop() {
  ...
  if (this->position_f_.has_value()) {
    auto s = (*this->position_f_)();
    if (s.has_value()) {
...
void TemplateCover::set_state_lambda(std::function<optional<CoverState>()> &&f) { this->state_f_ = f; }
...
void TemplateCover::control(const CoverCall &call) {
  if (call.get_command().has_value()) {
    ...
    optional<CoverState> new_state{};
    switch (cmd) {
      case COVER_COMMAND_OPEN:
        trigger_ = this->open_trigger_;
        new_state = COVER_OPEN;
        break;
      case COVER_COMMAND_CLOSE:
        trigger_ = this->close_trigger_;
        new_state = COVER_CLOSED;
        break;
      case COVER_COMMAND_STOP:
        trigger_ = this->stop_trigger_;
        break;
    }
    if (trigger_ != nullptr) {
      trigger_->trigger();
      this->prev_command_trigger_ = trigger_;

      if (new_state.has_value() && this->optimistic_) {
...
CoverTraits TemplateCover::traits() {
...
void TemplateCover::set_position_lambda(std::function<optional<float>()> &&position_f) {
  this->position_f_ = std::move(position_f);
}
