...
void PPD42XComponent::loop() {
  ...
  uint32_t duration_pl_02_5 =
      pulseIn(this->pl_02_5_sensor_->pl_->get_pin(), uint8_t(!this->pl_02_5_sensor_->pl_->is_inverted()),
              this->timeout_us_);
  const GPIOInputPin pinb = this->pl_10_0_sensor_->pl_
  uint32_t duration_pl_10_0 =
      pulseIn(this->pl_10_0_sensor_->pl_->get_pin(), uint8_t(!this->pl_10_0_sensor_->pl_->is_inverted()),
              this->timeout_us_);
