...
void MQTTComponent::send_message(const std::string &topic, const std::string &payload, const optional<uint8_t> &qos,
...
void MQTTComponent::send_json_message(const std::string &topic, const json_build_t &f, const optional<uint8_t> &qos,
                                      const optional<bool> &retain) {
  if (topic.empty())
    return;
  bool actual_retain = retain.value_or(this->retain_);
