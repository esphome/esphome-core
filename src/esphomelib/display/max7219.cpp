...
float MAX7219Component::get_setup_priority() const {
...
void MAX7219Component::send_byte_(uint8_t register_, uint8_t data) {
  this->write_byte(register_);
  this->write_byte(data);
}
void MAX7219Component::send_to_all_(uint8_t register_, uint8_t data) {
  this->enable();
  for (uint8_t i = 0; i < this->num_chips_; i++)
    this->send_byte_(register_, data);
  this->disable();
}
bool MAX7219Component::msb_first() {
  return true;
}
...
uint8_t MAX7219Component::print(uint8_t start_pos, const char *str) {
  ...
  for (; *str != '\0'; str++) {
    ...
    if (data == MAX7219_UNKNOWN_CHAR) {
      ESP_LOGW(TAG, "Encountered character '%c' with no MAX7219 representation while translating string!",
               *str);
    }
    if (*str == '.') {
      if (pos != start_pos)
...
uint8_t MAX7219Component::print(const char *str) {
...
uint8_t MAX7219Component::print(uint8_t pos, const std::string &str) {
  return this->print(pos, str.c_str());
}
uint8_t MAX7219Component::print(const std::string &str) {
  return this->print(0, str.c_str());
}
void MAX7219Component::set_intensity(uint8_t intensity) {
  this->intensity_ = intensity;
}
void MAX7219Component::set_num_chips(uint8_t num_chips) {
  this->num_chips_ = num_chips;
}

#ifdef USE_TIME
uint8_t MAX7219Component::strftime(uint8_t pos, const char *format, time::EsphomelibTime time) {
  char buffer[64];
...
uint8_t MAX7219Component::strftime(const char *format, time::EsphomelibTime time) {
