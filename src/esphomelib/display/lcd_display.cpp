...
#ifdef USE_TIME
void LCDDisplay::strftime(uint8_t column, uint8_t row, const char *format, time::EsphomelibTime time) {
  char buffer[64];
  size_t ret = time.strftime(buffer, sizeof(buffer), format);
  if (ret > 0)
    this->print(column, row, buffer);
}
void LCDDisplay::strftime(const char *format, time::EsphomelibTime time) {
...
void GPIOLCDDisplay::set_enable_pin(const GPIOOutputPin &enable) {
  this->enable_pin_ = enable.copy();
}
void GPIOLCDDisplay::set_rs_pin(const GPIOOutputPin &rs) {
  this->rs_pin_ = rs.copy();
}
void GPIOLCDDisplay::set_rw_pin(const GPIOOutputPin &rw) {
  this->rw_pin_ = rw.copy();
}
bool GPIOLCDDisplay::is_four_bit_mode_() {
